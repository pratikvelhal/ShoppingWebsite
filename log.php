...
<?php
	...
	$servername = "localhost";
							...
							$cart = $_GET['cart'];
							$sql2 = "SELECT password from $dbtname where username ='$username'";
    ...
    while($row = $result->fetch_assoc()) {
    	...
    	{							//	session_start();
    									...
    									$sql1 = "DELETE FROM $tname WHERE 1";
...
$result1 = $conn->query($sql);
    ...
    if($cart == '1' && $flag1 == true)
    {	echo 'Your Registered Address: ';
    	$sql2 = "SELECT address from $dbtname where username ='$username'";
    	$result = $conn->query($sql2);
    	 while($row = $result->fetch_assoc()) {
    	 echo $row["address"];
    	 $var = $row["address"];
    echo	' </br></br>
<p style="font-size:1.4em;">Please select address for item delivery.</p>


<form action="orderdetails.php?username=<?php echo $username;?>"  method="post">
	
	<input type="radio" name="address" value="Default"checked> Use Registered Address</input></br></br>
	<input type="radio" name="address" value = "new"> New Shipping Address: <div class="finalLog"><input type="text" name="newaddr"></div></input></br>

<div class="button1">
					   <p><input type="submit" name="Submit" value="Check Out"></p>
					 </div>
